...
**问题**：飞书 WebSocket 长连接可能因网络抖动、NAT 超时等断开，但进程不一定退出（僵死状态）——launchd 以为进程还活着就不会重启，导致飞书发消息无人响应。

**方案**：桥内置探活看门狗，每隔固定间隔主动调一次飞书轻量 API（`GET /bot/v3/info/`，SDK 自动带认证）。成功表示连接正常；失败则 `process.exit(1)`，launchd 自动拉起新进程重建连接。

| 配置 | 默认值 | 说明 |
| --- | --- | --- |
| `FEISHU_PROBE_INTERVAL_MS` | `3600000`（1 小时） | 探活间隔，可在 `.env` 中自定义 |

- 日志里会打印 `探活成功 (bot=xxx)` 或 `探活失败，退出以触发 launchd 重启: xxx`
- 定时器 `unref()`，不阻止进程退出
