...
- 后端：Node.js >= 20 + Express 5 + LangChain
- 前端：原生 Web（Cytoscape + ECharts 可视化）
- 向量库：Vectra（本地）
- LLM：OpenAI-compatible（key 在前端“注册 LLM”处填入）
- 无需 Python
...
src/
├── backend/
│   ├── app/        # 路由 + server (Express)
│   │   ├── server.mjs                            # 入口
│   │   ├── frontendRoutes.mjs                    # 前端路由
│   │   ├── llmRoutes.mjs                         # LLM 注册
│   │   ├── preParseTextbook2JSONRoutes.mjs       # 教材预解析
│   │   ├── parseEntityInTextbookJSON2VisualNodeRoutes.mjs  # 知识点抽取
│   │   ├── NodesDeduplicationAndAlignmentRoutes.mjs        # 跨教材整合
│   │   └── ragRoutes.mjs                         # RAG 问答
│   └── domain/     # 业务核心
│       ├── LLM/
│       ├── preParseTextbook2JSON/                # PDF→JSON 解析
│       ├── parseEntityInTextbookJSON2VisualNode/ # 知识点/关系抽取
│       ├── NodesDeduplicationAndAlignment/      # 跨教材去重整合
│       └── RAG/                                  # 向量检索 + BM25 混合
└── fronted/        # 原生 Web 前端（注意拼写：fronted）
    ├── index.html
    ├── app.js
    ├── knowledgeGraphView.mjs   # Cytoscape 图谱
    ├── graphInsightsView.mjs    # 洞察/关系矩阵
    ├── graphLayout.mjs
    ├── dedupePolicy.mjs
    └── styles.css
```
